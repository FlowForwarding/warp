...
*/
public class BufferedBinaryNonEncoder extends BinaryEncoder {
 private byte[] buf;
 private int pos;
 private ByteSink sink;
 private int bulkLimit;

 BufferedBinaryNonEncoder(OutputStream out, int bufferSize) {
    configure(out, bufferSize);
 }

 BufferedBinaryNonEncoder configure(OutputStream out, int bufferSize) {
    if (null == out)
 ...
 public void writeInt(int n) throws IOException {
    ensureBounds(5);
    pos += BinaryData.nonEncodeInt(n, buf, pos);
 }
