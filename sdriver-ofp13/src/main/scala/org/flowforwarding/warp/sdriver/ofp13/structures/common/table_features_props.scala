...
import org.flowforwarding.warp.sdriver.ofp13.structures.ofp_table_feature_prop_type._
...
case class ofp_table_feature_prop_instructions private [sdriver] (
...
object ofp_table_feature_prop_instructions extends TFPHelper{
  private [sdriver] def build(instruction_ids: RawSeq[ofp_instruction_id]) =
...
case class ofp_table_feature_prop_instructions_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_next_tables private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
object ofp_table_feature_prop_next_tables extends TFPHelper{
  private [sdriver] def build(next_table_ids: RawSeq[UInt8]) =
...
case class ofp_table_feature_prop_next_tables_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_write_actions private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
object ofp_table_feature_prop_write_actions extends TFPHelper{
  private [sdriver] def build(action_ids: RawSeq[ofp_action_id]) =
...
case class ofp_table_feature_prop_write_actions_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_apply_actions private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_apply_actions_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_experimenter private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
...
object ofp_table_feature_prop_experimenter extends TFPHelper{
  ...
  private [sdriver] def build(experimenter_data: RawSeq[UInt8], experimenter: UInt32, exp_type: UInt32) =
...
case class ofp_table_feature_prop_experimenter_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  experimenter: UInt32, /* Experimenter ID which takes the same form as in struct ofp_experimenter_header. */
  exp_type: UInt32, /* Experimenter defined. */
  /* Followed by:
...
case class ofp_table_feature_prop_match private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
object ofp_table_feature_prop_match extends TFPHelper{
  private [sdriver] def build(oxm_ids: RawSeq[UInt32]) =
...
case class ofp_table_feature_prop_wildcards private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_write_setfield private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_write_setfield_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_apply_setfield private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
...
case class ofp_table_feature_prop_apply_setfield_miss private [sdriver] (
  len: UInt16,/* Length in bytes of this property. */
  /* Followed by:
  * - Exactly (length - 4) bytes containing the instruction ids, then
  * - Exactly (length + 7)/8*8 - (length) (between 0 and 7)
