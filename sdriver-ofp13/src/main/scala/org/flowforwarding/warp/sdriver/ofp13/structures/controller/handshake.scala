...
import org.flowforwarding.warp.sdriver.ofp13.structures.ofp_capabilities.OFP_CAPABILITIES
import org.flowforwarding.warp.sdriver.dynamic.DynamicPath

case class ofp_switch_features_request private [sdriver](header: ofp_header)

object ofp_switch_features_request{
  private [sdriver] def build(@DynamicPath("header", "xid") xid: UInt32) = ofp_switch_features_request(ofp_header(ofp_length.OFPL_FEATURES_REQUEST_LEN, xid))
