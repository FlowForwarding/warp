...
case class echo_request private[sdriver] (header: ofp_header, elements: RawSeq[UInt8] = RawSeq())/* Arbitrary-length data field */

object echo_request extends RawSeqFieldsInfo{
  val rawFieldsLengthCalculator: LengthCalculator = { case 1 => bodyLength }

  private[sdriver] def build(@DynamicPath("header", "xid") xid: UInt32, elements: RawSeq[UInt8] = RawSeq()) = {
...
case class echo_reply private[sdriver] (header: ofp_header, elements: RawSeq[UInt8])
