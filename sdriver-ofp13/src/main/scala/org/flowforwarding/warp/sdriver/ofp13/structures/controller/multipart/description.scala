...
import org.flowforwarding.warp.sdriver.ofp13.structures.ofp_multipart_type.OFP_MULTIPART_TYPE
...
case class ofp_multipart_desc_request private [sdriver] () extends EmptyMultipartRequest {
  override def tp: OFP_MULTIPART_TYPE = ofp_multipart_type.OFPMP_DESC
}

object ofp_multipart_desc_request {
  private [sdriver] def build() = ofp_multipart_desc_request()
}

case class ofp_multipart_desc_reply private [sdriver] (desc: ofp_desc)