...
import org.flowforwarding.warp.sdriver.ofp13.structures.ofp_flow_mod_flags.OFP_FLOW_MOD_FLAGS
...
case class ofp_flow_stats_request private [sdriver](
...
case class ofp_multipart_flow_stats_request private [sdriver] (structure: ofp_flow_stats_request) extends MultipartRequest[ofp_flow_stats_request] {
...
object ofp_multipart_flow_stats_request {
  private [sdriver] def build(structure: ofp_flow_stats_request) = ofp_multipart_flow_stats_request(structure)
}

case class ofp_multipart_flow_stats_reply private [sdriver] (stats: RawSeq[ofp_flow_stats])