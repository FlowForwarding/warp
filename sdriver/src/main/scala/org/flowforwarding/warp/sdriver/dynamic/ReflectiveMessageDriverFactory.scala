...
import org.flowforwarding.warp.sdriver.{StaticDriver, OfpMsg}
import org.flowforwarding.warp.driver_api.dynamic.DynamicMessageDriverFactory
...
class ReflectiveMessageDriverFactory(definitionPaths: Array[String]) extends DynamicMessageDriverFactory[ReflectiveStructure] with StrictLogging{
  ...
  def get(versionCode: UByte): ReflectiveMessageDriver =
    (definedDrivers collectFirst { case (d, classes) if d.versionCode == versionCode => d.toReflectiveDriver(classes)}).get
