...
case class ofp_multipart_meter_features_request private [protocol] () extends EmptyMultipartRequest {
  override def tp: OFP_MULTIPART_TYPE = ofp_multipart_type.OFPMP_METER_FEATURES
}

object ofp_multipart_meter_features_request {
  private [protocol] def build() = ofp_multipart_meter_features_request()
}

case class ofp_multipart_meter_features_reply private [protocol] (features: ofp_meter_features)