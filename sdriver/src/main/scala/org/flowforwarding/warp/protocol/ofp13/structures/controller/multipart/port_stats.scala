...
case class ofp_port_stats_request private[protocol] (
...
object ofp_port_stats_request{
  private[protocol] def build(port_no: OFP_PORT_NO) = ofp_port_stats_request(port_no, Pad4())
...
case class ofp_port_stats private[protocol] (
  port_no: OFP_PORT_NO,
...
case class ofp_multipart_port_stats_request private [protocol] (structure: ofp_port_stats_request) extends MultipartRequest[ofp_port_stats_request] {
...
object ofp_multipart_port_stats_request {
  private [protocol] def build(structure: ofp_port_stats_request) = ofp_multipart_port_stats_request(structure)
}

case class ofp_multipart_port_stats_reply private [protocol] (stats: ofp_port_stats)