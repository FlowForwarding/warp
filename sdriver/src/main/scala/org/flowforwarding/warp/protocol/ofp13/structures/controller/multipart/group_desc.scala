...
case class ofp_group_desc private[protocol] (
...
case class ofp_multipart_group_desc_request private [protocol] () extends EmptyMultipartRequest {
  override def tp: OFP_MULTIPART_TYPE = ofp_multipart_type.OFPMP_GROUP_DESC
}

object ofp_multipart_group_desc_request {
  private [protocol] def build() = ofp_multipart_group_desc_request()
}

case class ofp_multipart_group_desc_reply private [protocol] (desc: RawSeq[ofp_group_desc])