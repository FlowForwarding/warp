...
case class ofp_experimenter_multipart private[protocol] (
...
object ofp_experimenter_multipart extends RawSeqFieldsInfo{
  ...
  private[protocol] def build(experimenter: UInt32,
                              exp_type: UInt32,
                              data: RawSeq[UInt8]) =
...
case class ofp_multipart_experimenter_request private [protocol] (structure: ofp_experimenter_multipart) extends MultipartRequest[ofp_experimenter_multipart] {
...
object ofp_multipart_experimenter_request {
  private [protocol] def build(structure: ofp_experimenter_multipart) = ofp_multipart_experimenter_request(structure)
}

case class ofp_multipart_experimenter_reply private [protocol] (data: ofp_experimenter_multipart)