...
case class ofp_table_stats private [protocol](
...
case class ofp_multipart_table_stats_request private [protocol] () extends EmptyMultipartRequest {
  override def tp: OFP_MULTIPART_TYPE = ofp_multipart_type.OFPMP_TABLE
}

object ofp_multipart_table_stats_request {
  private [protocol] def build() = ofp_multipart_table_stats_request()
}

case class ofp_multipart_table_stats_reply private [protocol] (stats: RawSeq[ofp_table_stats])