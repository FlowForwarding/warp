...
case class ofp_action_id_copy_ttl_out private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_copy_ttl_out {
  private [protocol] def build() = ofp_action_id_copy_ttl_out(AL_COPY_TTL_OUT)
}

case class ofp_action_id_copy_ttl_in private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_copy_ttl_in {
  private [protocol] def build() = ofp_action_id_copy_ttl_in(AL_COPY_TTL_IN)
}

case class ofp_action_id_dec_mpls_ttl private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_dec_mpls_ttl {
  private [protocol] def build() = ofp_action_id_dec_mpls_ttl(AL_DEC_MPLS_TTL)
}

case class ofp_action_id_pop_vlan private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_pop_vlan {
  private [protocol] def build() = ofp_action_id_pop_vlan(AL_POP_VLAN)
}

case class ofp_action_id_dec_nw_ttl private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_dec_nw_ttl {
  private [protocol] def build() = ofp_action_id_dec_nw_ttl(AL_DEC_NW_TTL)
}

case class ofp_action_id_pop_pbb private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_pop_pbb {
  private [protocol] def build() = ofp_action_id_pop_pbb(AL_POP_PBB)
}

case class ofp_action_id_output private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_output {
  private [protocol] def build() = ofp_action_id_output(AL_OUTPUT)
}

case class ofp_action_id_group private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_group {
  def build() = ofp_action_id_group(AL_GROUP)
}

case class ofp_action_id_set_queue private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
...
case class ofp_action_id_nw_ttl private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_nw_ttl {
  def build() = ofp_action_id_nw_ttl(AL_SET_NW_TTL)
}

// type ==  = OFPAT_PUSH_VLAN/MPLS/PBB, len too
case class ofp_action_id_push_vlan private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_push_vlan {
  def build() = ofp_action_id_push_vlan(AL_PUSH_VLAN) }

case class ofp_action_id_push_mpls private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_push_mpls {
  def build() = ofp_action_id_push_mpls(AL_PUSH_MPLS)
}

case class ofp_action_id_push_pbb private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_push_pbb {
  def build() = ofp_action_id_push_pbb(AL_PUSH_PBB)
}

case class ofp_action_id_pop_mpls private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_pop_mpls {
  def build() = ofp_action_id_pop_mpls(AL_POP_MPLS)
}

case class ofp_action_id_set_field private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_set_field{
  private [protocol] def build() = ofp_action_id_set_field(AL_SET_FIELD) // TODO ??
}

case class ofp_action_id_experimenter private [protocol] (len: OFP_ACTION_LENGTH) extends ofp_action_id
object ofp_action_id_experimenter_header {
  private [protocol] def build() = ofp_action_id_experimenter(AL_EXPERIMENTER)
}
