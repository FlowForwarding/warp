...
import org.flowforwarding.warp.protocol.ofp13.structures.ofp_match_type.OFP_MATCH_TYPE
import spire.math.{UByte, UShort}
import org.flowforwarding.warp.protocol.ofp13.structures.oxm_ofb_match_field.OXM_OFB_MATCH_FIELD

/* Fields to match against flows */
case class ofp_match private[protocol] (
...
object ofp_match extends RawSeqFieldsInfo{
  private[protocol] def build(mType: OFP_MATCH_TYPE, fields: RawSeq[oxm_tlv]) ={
  ...
  private[protocol] def totalLength(m: ofp_match) = {
...
case class oxm_tlv private[protocol] (header: UInt32, entry: RawSeq[UInt8])
