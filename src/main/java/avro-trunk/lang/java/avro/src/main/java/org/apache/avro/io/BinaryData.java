...
public class BinaryData {
  ...
  /** Writes an integer to the byte array 'as-is' at the given position. 
   * Will throw IndexOutOfBounds if it overflows. Users should ensure 
   * that there are at least 5 bytes left in the buffer before calling 
   * this method. 
   * @return The number of bytes written to the buffer, between 1 and 5.
   */
  public static int nonEncodeInt(int n, byte[] buf, int pos) {
  // move sign to low-order bit, and flip others if negative
     int start = pos;
 // TODO DO add for more than 8-bits ints...
     buf[pos++] = (byte) (n >> 24);
     buf[pos++] = (byte) (n >> 16);
     buf[pos++] = (byte) (n >> 8);
     buf[pos++] = (byte) n;
