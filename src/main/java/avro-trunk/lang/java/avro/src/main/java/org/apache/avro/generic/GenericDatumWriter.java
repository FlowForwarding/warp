...
public class GenericDatumWriter<D> implements DatumWriter<D> {
  ...
  protected void write(Schema schema, Object datum, Encoder out)
    ...
    try {
      ...
      case BITMAP: writeBitmap(schema, datum, out); break;
  ...
  /** Called to write a bitmap.  May be overridden for alternate record
   * representations.*/
  protected void writeBitmap(Schema schema, Object datum, Encoder out)
    throws IOException {
    
    JsonNode result = schema.getOperation().result();
    out.writeInt(result.getIntValue());
