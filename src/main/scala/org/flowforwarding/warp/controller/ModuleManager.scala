...
private class ModuleManager(val bus: ControllerBus, manager: ActorRef) extends ControllerBusActor {
  ...
  def receive = {
    ...
    case Start(address) =>
      manager ! TcpMessage.bind(self, address, 100)
    ...
    case SetFactory(factoryClass, args) =>
      ...
      match {
        ...
        case Failure(t) =>
          sender ! SetFactoryResponse(Some(new Exception("The specified factory class does not have an appropriate constructor")))
      }
        ...
        case Success(module) =>
          module ? CheckCompatibility(driverFactory) onComplete {
            case Success(true) =>
              ...
              println(s"Module $moduleName started")
