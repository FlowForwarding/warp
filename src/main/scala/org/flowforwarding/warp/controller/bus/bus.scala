...
trait ServiceBus{
  ...
  def askService(requester: ActorRef, request: ServiceRequest): Future[Any] = {
    services collectFirst { // collect ???
      ...
      case None => Future.failed(new Exception("Service not found."))
    }
  }
}
...
trait ServiceBusActor extends Actor{
  ...
  protected final def askService(request: ServiceRequest): Future[Any] = bus.askService(self, request)
